...
For AWS with automatic IAM role trust policy configuration:

```hcl
locals {
  aws_account_id = "123456789012"
  role_name      = "databricks-service-credential"
}

resource "databricks_credential" "service_credential" {
  name            = "example_service_credential"
  purpose         = "SERVICE"
...
data "databricks_aws_unity_catalog_assume_role_policy" "this" {
  ...
  external_id    = databricks_credential.service_credential.aws_iam_role[0].external_id
}

resource "aws_iam_role" "service_credentials_role" {
...
- `external_id` - (Output only) The external ID used in the IAM role trust policy to prevent the confused deputy problem. This value is generated by Databricks and should be used when configuring the trust policy for the IAM role.
